...
// UPDATED RENDER LINK (Truncation)
function renderLink(d, t) {
    ...
    if(fullText.length > 15) {
        displayText = fullText.substring(0, 12) + '...';
    }
    ...
    return `<a href="${url}" target="_blank" class="tag-link" title="${fullText}">
        <i class="${icon}"></i> <span class="tag-truncate">${displayText}</span>
    </a>`;
}
...
function renderMetaList(){
    ...
    render(db.lists, 'list-manager-ul', 'lists');
    render(db.cats, 'cat-manager-ul', 'cats');
}
