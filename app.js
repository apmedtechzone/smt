...
// ==========================================
// Check your browser console to see if your old data is safe. 
// If you lost it, paste your backup 'const defaultDB = ...' here.
const defaultDB = {
...
document.addEventListener('DOMContentLoaded', () => {
    initDropdowns();
    initBulkRows(20); // Initialize with 20 rows for easier pasting
    renderTable();
    checkAdminStatus();
    
    // Attach the Smart Paste Listener
...
function saveDataLocally() {
    ...
    alert("Saved to this Browser! \nTo publish live, copy the code to GitHub.");
    const codeToPaste = `const defaultDB = ${JSON.stringify(db, null, 4)};\n\nlet db = JSON.parse(localStorage.getItem('amtz_db')) || defaultDB;`;
    navigator.clipboard.writeText(codeToPaste);
}

function hardReset() {
    if(confirm("Factory Reset? Wipes local changes.")) {
        localStorage.removeItem('amtz_db');
        location.reload();
    }
}
...
// --- SMART PASTE LOGIC (THE FIX) ---
function handleGridPaste(e) {
    // 1. Prevent default paste (which puts everything in one cell)
    e.preventDefault();

    // 2. Get Clipboard Data
    const clipboardData = (e.clipboardData || window.clipboardData).getData('text');
    
    // 3. Parse Data (Split rows by newline, columns by tab)
    const rows = clipboardData.split(/\r\n|\n|\r/).filter(row => row.trim() !== '');
    
    // 4. Find where the user clicked
    let target = e.target;
    if (target.tagName !== 'INPUT') return; // Only paste if clicked inside an input
    ...
    // 5. Loop through pasted data and fill inputs
    rows.forEach((rowData, rIdx) => {
        // Automatically add more rows if paste is larger than current table
        if (startRowIndex + rIdx >= tableBody.children.length) {
            addBulkRows(5);
        }
...
function analyzeBulkUpload() {
    ...
    if(pendingConflicts.length > 0) { renderConflicts(); document.getElementById('conflict-count').innerText=pendingConflicts.length; openModal('conflict-modal'); }
        ...
        alert(`Created: ${created} organizations.`); 
        renderTable(); 
        initBulkRows(20); 
        saveDataLocally(); 
    }
