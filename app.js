...
// This is the default data (used if no local data is found)
const defaultDB = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initBulkRows(10); 
    renderTable();
    checkAdminStatus(); // Check if session is still active
});
...
// ==========================================
// 3. PERSISTENCE (THE FIX)
// ==========================================
function saveDataLocally() {
    // 1. Save to Browser Memory (Persists after refresh)
    localStorage.setItem('amtz_db', JSON.stringify(db));
    
    // 2. Visual Feedback
    alert("Saved to this Browser! \n\nIMPORTANT: To update the live website for everyone else, paste the code from your clipboard into your app.js file on GitHub.");
    
    // 3. Copy Code for GitHub
    const codeToPaste = `const defaultDB = ${JSON.stringify(db, null, 4)};\n\n// LOAD STATE: Try to load from Browser Storage, otherwise use Default\nlet db = JSON.parse(localStorage.getItem('amtz_db')) || defaultDB;`;
    navigator.clipboard.writeText(codeToPaste);
}

function hardReset() {
    if(confirm("Factory Reset? This will wipe all local changes and revert to the original code.")) {
...
function analyzeBulkUpload() {
    ...
    if(pendingConflicts.length > 0) { renderConflicts(); document.getElementById('conflict-count').innerText=pendingConflicts.length; openModal('conflict-modal'); }
        ...
        renderTable(); 
        initBulkRows(10); 
        saveDataLocally(); // Auto-save after import
    }
}
...
function resolveConflict(id, action) {
    ...
    if(pendingConflicts.length===0) { 
        ...
        renderTable(); 
        alert("Resolved."); 
        saveDataLocally(); // Auto-save after resolve
    }
}
...
function login() { 
    if(document.getElementById('login-email').value==='saragadamteja.k@amtz.in' && document.getElementById('login-pass').value==='9989'){ 
        isAdmin=true; 
        localStorage.setItem('amtz_admin', 'true'); // Keep logged in
        updateUIForAdmin();
...
function copyConfig(){ saveDataLocally(); } // Button now triggers save
